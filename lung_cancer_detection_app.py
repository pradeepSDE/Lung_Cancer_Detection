...
model = load_model('lung_cancer_vgg16_model_1.h5')

# Define class names
# classes = ['adenocarcinoma', 'squamous', 'benign']
...
uploaded_file = st.file_uploader(
    "Please upload a histopathology image", type=["jpg", "png", "jpeg"])
...
if uploaded_file is not None:
    ...
    if st.button('Classify Image'):
        ...
        st.write(f"Benign: {probabilities[1]:.4f}")
        st.write(f"Squamous: {probabilities[2]:.4f}")
        st.write(f"Adenocarcinoma: {probabilities[0]:.4f}")
